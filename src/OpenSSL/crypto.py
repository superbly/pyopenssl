...
class PKCS7(object):
    ...
    def type_is_encrypted(self):
        """
        Check if this NID_pkcs7_encrypted object

        :returns: True if the PKCS7 is of type encrypted
        """
    ...
    def type_is_digest(self):
        """
        Check if this NID_pkcs7_digest object

        :returns: True if the PKCS7 is of type digest
        """
        return bool(_lib.PKCS7_type_is_digest(self._pkcs7))
    ...
    def get_certificates(self):
        ...
        for i in range(_lib.sk_X509_num(certs)):
            ...
            pycert._x509 = _ffi.gc(
                    _lib.X509_dup(_lib.sk_X509_value(certs, i)),
                    _lib.X509_free)
    ...
    def get_crls(self):
        ...
        for i in range(_lib.sk_X509_CRL_num(crls)):
            ...
            pycrl._crl = _ffi.gc(_lib.X509_CRL_dup(
                _lib.sk_X509_CRL_value(crls, i)), _lib.X509_CRL_free)
            pycrls.append(pycrl)
        if not pycrls:
            return None
        return tuple(pycrls)
